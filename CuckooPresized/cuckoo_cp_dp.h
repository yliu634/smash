...
class ControlPlaneCuckoo {
public:
  static const char DL = digestLength > 0 ? digestLength : sizeof(Match) * 8;
  static const Match MASK = (1 << DL) - 1;
  
  friend class DataPlaneCuckoo<Key, Value, Match, DL, kCandidateBuckets, kSlotsPerBucket>;
  ...
  explicit ControlPlaneCuckoo(uint32_t num_entries = 64) {
    Clear(num_entries);
    for (auto &hash : h) {
      hash.setSeed(rand());
    }
  ...
  const Key *insert(const Key &k, const Value &v, vector<CuckooMove> *const path = 0) {
      ...
      for (int i = 0; i < kCandidateBuckets; ++i) {
        ...
        for (const Key &key : set) {
          if ((getDigest(key) & MASK) == (getDigest(k) & MASK)) {
            Clocker::count("Cuckoo collision");
    ...
    if (target_slot != -1) {
      if (full_debug) Clocker::count("Cuckoo direct insert");
    ...
    if (full_debug) Clocker::count("Cuckoo cuckoo insert");
    
    if (CuckooInsert<rememberPath>(k, v, path)) {
      return &k;
    } else {
      if (full_debug) Clocker::count("Cuckoo insert fail");
      entryCount--;
      return nullptr;
    }
  }
...
//  }
  ...
  template<class NV>
  ControlPlaneCuckoo<Key, NV, Match, maintainCollisionSet, DL, kCandidateBuckets, kSlotsPerBucket>
  Compose(unordered_map<Value, NV> &migrate) {
    ControlPlaneCuckoo<Key, NV, Match, maintainCollisionSet, DL, kCandidateBuckets, kSlotsPerBucket> other;
...
public:
  explicit DataPlaneCuckoo(
      const ControlPlaneCuckoo<Key, Value, Match, true, DL, kCandidateBuckets, kSlotsPerBucket> &controlPlane
  ...
  explicit DataPlaneCuckoo(
      const ControlPlaneCuckoo<Key, Value, Match, false, DL, kCandidateBuckets, kSlotsPerBucket> &controlPlane
  ) : num_buckets_(controlPlane.num_buckets_) {
    for (int i = 0; i < kCandidateBuckets + 1; ++i) {
      h[i] = controlPlane.h[i];
    }
