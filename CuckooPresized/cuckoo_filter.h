...
//#include "../common.h"
//
//template<class Key, char digestLength = 4, int kCandidateBuckets = 2, int kSlotsPerBucket = 4>
//class CuckooFilter {
//public:
//  typedef uint16_t Match;
//
...
//      InsertInternal(k, target_bucket, target_slot);
//      return &k;
//    }
//
...
//      return &k;
//    } else {
//      Clocker::count("Cuckoo", "cuckoo insert fail");
//      entryCount--;
//      return nullptr;
//    }
//  }
...
//  inline bool Find(const Key &k) const {
//    for (int i = 0; i < kCandidateBuckets; ++i) {
//      uint32_t bucket = fast_map_to_buckets(h[i](k));
//      if (FindInBucket(k, bucket)) return true;
//    }
//
//    return false;
//  }
...
//  };
//
//  inline void setDigest(int bid, int slot, Match digest) {
//    buckets_[bid].digests[slot] = digest & MASK;
//  }
//
//  inline void setDigest(Bucket &bdst, int sdst, const Bucket &bsrc, int ssrc) {
...
//
//  inline void InsertInternal(const Key &k, uint32_t b, int slot) {
//    Bucket &bref = buckets_[b];
//    setDigest(b, slot, getDigest(k));
//
...
//  // For the associative cuckoo table, check all of the slots in
//  // the bucket to see if the key is present.
//  bool FindInBucket(Match digest, uint32_t b) const {
//    const Bucket &bref = buckets_[b];
//    for (int i = 0; i < kSlotsPerBucket; i++) {
//      if ((bref.occupiedMask & (1U << i)) && (bref.keyDigests[i] & MASK) == (digest & MASK)) {
//        return true;
//      }
//    }
//    return false;
...
//          entry = parent;
//        }
//        InsertInternal(k, entry.bucket, free_slot);
//        return true;
